...
<div x-data="{ photo:false, url:null }" class="space-y-6">
    ...
    {{-- INFO CARD --}}
    <div class="bg-white p-6 rounded-2xl shadow-md border grid grid-cols-1 md:grid-cols-2 gap-4 text-base">
        <div>
    ...
    {{-- BUTTON VALIDASI (hanya user method & menunggu validasi) --}}
    @if($survey->survey_method == 'user' && $survey->status == 'menunggu_validasi')
        <div class="flex gap-3">
            <form method="POST" action="{{ route('admin.surveys.approve', $survey) }}">
                @csrf
                <button
                    class="px-5 py-2 bg-green-600 text-white rounded-xl shadow hover:bg-green-700 font-semibold">
                    Setujui Survey
                </button>
            </form>

            <form method="POST" action="{{ route('admin.surveys.reject', $survey) }}">
                @csrf
                <button
                    class="px-5 py-2 bg-red-600 text-white rounded-xl shadow hover:bg-red-700 font-semibold">
                    Tolak Survey
                </button>
            </form>
        </div>
...
{{-- TABEL HASIL SURVEY --}}
<div class="bg-white p-3 rounded-2xl shadow-md border">
